...
export const HomePage: React.FC<HomePageProps> = ({ onExploreClick, onOpenQuote, onSelectProject }) => {
  ...
  useEffect(() => {
    ...
    if (heroSection) {
      ...
      if (false) { // reduced-motion disabled
        ...
        // Background cinematic — gentle, centered
        if (heroBg) {
          gsap.fromTo(
            heroBg,
            { scale: 1.14, opacity: 0, filter: 'blur(6px)' },
            { scale: 1, opacity: 0.72, filter: 'blur(0px)', duration: 1.6, ease: 'power3.out' }
          );
        }
        // Pill — subtle fade
        const heroPillEl = heroSection.querySelector('#hero-text-layer > span.reveal-entry') as HTMLElement | null;
        if (heroPillEl) {
          gsap.set(heroPillEl, { opacity: 0, y: 14 });
          gsap.to(heroPillEl, { opacity: 1, y: 0, duration: 0.6, ease: 'power3.out', delay: 0.2 });
        }
        // Hero lines: clip + y reveal — most noticeable, centered
        ...
        if (heroSub) {
          ...
          gsap.to(heroSub, { opacity: 1, y: 0, duration: 0.8, ease: 'power3.out', delay: 0.85 });
        }
        // CTAs + trust chips stagger after
        const heroCtas = heroSection.querySelectorAll<HTMLElement>('#hero-text-layer .reveal-entry');
        // Filter out pill and h1 lines already handled, keep CTAs and trust
        const ctaAndTrust = Array.from(heroCtas).filter((el) => !el.classList.contains('hero-line') && el !== heroPillEl);
        gsap.set(ctaAndTrust, { opacity: 0, y: 18 });
        gsap.to(ctaAndTrust, {
          opacity: 1,
          y: 0,
          duration: 0.7,
  ...
  useEffect(() => {
    const prefersReduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    const intensity = 1; // reduced-motion commented out
  ...
  return (
    <div ref={rootRef} className="w-full max-w-full min-w-0 overflow-x-clip bg-[#fff8f3]">
      ...
      {/* Everyday Services — photo-hero cards — intentional swipe rail on <640 with peek, grid at sm+ */}
      <section className="w-full max-w-full overflow-x-clip bg-[#fff8f3] py-20 md:py-24">
        <div className="max-w-[1200px] mx-auto px-5 sm:px-6 md:px-12 min-w-0 max-w-full">
          ...
          <div className="relative -mx-5 sm:mx-0 min-w-0 max-w-full">
            <div aria-hidden="true" className="pointer-events-none absolute left-0 top-0 bottom-0 w-6 bg-gradient-to-r from-[#fff8f3] to-transparent z-10 sm:hidden" />
            <div aria-hidden="true" className="pointer-events-none absolute right-0 top-0 bottom-0 w-8 bg-gradient-to-l from-[#fff8f3] to-transparent z-10 sm:hidden" />
            <div className="flex gap-4 overflow-x-auto snap-x snap-mandatory scroll-px-5 px-5 pb-4 -mb-4 sm:grid sm:grid-cols-2 lg:grid-cols-4 sm:gap-8 sm:overflow-visible sm:px-0 sm:pb-0 sm:mb-0 sm:scroll-px-0 no-scrollbar rail-scroll min-w-0 max-w-full">
            <Link
              to="/services"
      ...
      {/* Recent jobs around Lagos — intentional swipe rail on <640 with peek, stack-safe, no page overflow */}
      <section id="recent-jobs" className="w-full max-w-full overflow-x-clip bg-[#f9f2ed] py-20 md:py-24">
        <div className="max-w-[1200px] mx-auto px-5 sm:px-6 md:px-12 min-w-0 max-w-full">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4 reveal-entry min-w-0">
            <div className="min-w-0">
          ...
          <div className="relative -mx-5 sm:mx-0 min-w-0 max-w-full">
            <div aria-hidden="true" className="pointer-events-none absolute left-0 top-0 bottom-0 w-6 bg-gradient-to-r from-[#f9f2ed] to-transparent z-10 sm:hidden" />
            <div aria-hidden="true" className="pointer-events-none absolute right-0 top-0 bottom-0 w-8 bg-gradient-to-l from-[#f9f2ed] to-transparent z-10 sm:hidden" />
            <div className="flex gap-4 overflow-x-auto snap-x snap-mandatory scroll-px-5 px-5 pb-4 -mb-4 sm:grid sm:grid-cols-2 md:grid-cols-3 sm:gap-8 sm:overflow-visible sm:px-0 sm:pb-0 sm:mb-0 sm:scroll-px-0 no-scrollbar rail-scroll min-w-0 max-w-full">
            {/* Master Ensuite Refit — Ikoyi */}
            <button
              onClick={() => onSelectProject(PROJECTS[2] ?? PROJECTS[0])}
              className="reveal-entry bg-white rounded-2xl overflow-hidden shadow-sm flex flex-col group hover:shadow-md transition-shadow text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-[#a43716] cursor-pointer border border-black/5 min-w-0 max-w-full w-[86%] min-w-[300px] max-w-[360px] snap-start shrink-0 sm:w-auto sm:min-w-0 sm:max-w-none sm:shrink"
            >
              <div className="aspect-[4/3] w-full max-w-full min-w-0 overflow-hidden bg-[#f3ede7] relative">
                <img
      ...
      {/* How We Work — Four simple rules — stacks vertically at 390 (grid-cols-1), no swipe rail */}
      <section className="w-full max-w-full overflow-x-clip bg-[#fff8f3] py-20 md:py-24">
        <div className="max-w-[1200px] mx-auto px-5 sm:px-6 md:px-12 min-w-0 max-w-full">
              ...
              <form className="space-y-4 min-w-0 max-w-full" onSubmit={handleBookingSubmit}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 min-w-0 max-w-full">
                  <div className="min-w-0 max-w-full">
                    ...
                    <input
                      ...
                      className="w-full max-w-full min-w-0 h-12 px-4 rounded-xl bg-[#f3ede7] border-0 text-[#1d1b18] placeholder:text-[#8b716a] focus:outline-none focus:ring-2 focus:ring-[#a43716] text-[14px]"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 min-w-0 max-w-full">
                  <div className="min-w-0 max-w-full">
                    ...
                    <select
                      ...
                      className="w-full max-w-full min-w-0 h-12 px-4 rounded-xl bg-[#f3ede7] border-0 text-[#1d1b18] focus:outline-none focus:ring-2 focus:ring-[#a43716] text-[14px] truncate"
                    >
                ...
                <div className="min-w-0 max-w-full">
                  ...
                  <textarea
                    ...
                    className="w-full max-w-full min-w-0 p-4 rounded-xl bg-[#f3ede7] border-0 text-[#1d1b18] placeholder:text-[#8b716a] focus:outline-none focus:ring-2 focus:ring-[#a43716] text-[14px] resize-none"
      ...
      {/* FAQ 3 cols */}
      <section className="w-full max-w-full overflow-x-clip bg-[#fff8f3] py-16 md:py-20">
        <div className="max-w-[1200px] mx-auto px-5 sm:px-6 md:px-12 min-w-0 max-w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto min-w-0 max-w-full">
